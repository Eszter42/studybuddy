            ...
            <div class="bg-white shadow rounded p-6">
                <form method="POST" action="{{ route('tasks.update', $task) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="title" value="{{ old('title', $task->title) }}" class="w-full rounded border-gray-300" required>
                        @error('title') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
                    </div>

                    <div>
                        <label class="text-sm text-gray-600">Description</label>
                        <textarea name="description" class="w-full rounded border-gray-300" rows="4">{{ old('description', $task->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="subject_id" class="w-full rounded border-gray-300">
                                ...
                                @foreach($subjects as $s)
                                    <option value="{{ $s->id }}" @selected(old('subject_id',$task->subject_id)==$s->id)>{{ $s->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="estimate_minutes" value="{{ old('estimate_minutes', $task->estimate_minutes) }}" class="w-full rounded border-gray-300" min="1" max="1440">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('tasks.show', $task) }}" class="px-4 py-2 rounded bg-gray-100 text-gray-900">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
